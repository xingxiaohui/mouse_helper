...
path = os.path.abspath(os.path.dirname(__file__)).split('mouse_helper')[0] + str('mouse_helper\\image\\')
...
def linshi():
    # path = 'F:/image/troops/'
    images = config.linshi_image_names
    for img in images:
        # 注意 需要针对不同的图片进行不同的鼠标点击偏移
        if img in jieshu:
            utils.find_and_click(path + 'linshi_' + str(img), 30)
        else:
            utils.find_and_click(path + 'linshi_' + str(img), 5)